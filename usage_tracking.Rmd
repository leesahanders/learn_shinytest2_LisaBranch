...
Sys.setenv(CONNECT_API_KEY = "Add_your_key_here")

# Could also do this interactively so the API key isn't exposed:  Sys.setenv("RSTUDIO_CONNECT_API_KEY" = rstudioapi::askForPassword("Enter Connect Token:")) 
```
...
#### Get all user ID's so we can translate them later ####
...
# I think rsc needs to have an initial query so this won't fail, could probably be collapsed with above line
...
# I think this is getting all usage on the server, but it's defaulting to just usage for my content. Need to figure out how to add filter to a specific content and/or the translation of the ID so we can filter later (less efficient)
shiny_usage <- map_dfr(
  rsc_shiny$results,
  ~ tibble(
      guid = .x$content_guid,
